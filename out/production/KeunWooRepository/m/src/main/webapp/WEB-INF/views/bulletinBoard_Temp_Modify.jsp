...
<script>
	...
	function fnSave() {
		...
		$.ajax({
			...
			url : "getBullentinBoardPwChk", //요청할 url, 주소:포트(http://localhost:8080)는 일반적으로 생략
			...
			success : function(data) { //데이터 송,수신에 성공했을 경우의 동작
				...
				if (data.PASSWORD == "X") { /* 비밀번호가 틀렸을 경우  */
					...
					var t = confirm("게시물을 수정하시겠습니까?");
					if (t == true) {
						location.href = "/bulletinBoard_Modify?no=" + no;
...
<body>
	<input type="hidden" value="${post_Temp.NO}" id="no">
		...
		<div class="no_member_cont">
			<div class="inner">
				...
				<div class="btn_box">
					...
					<button type="button" onclick="fnSave()" class="btn_blue small">확인</button>
				</div>
			</div>
		</div>
	</div>
